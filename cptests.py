...
class CPDALSTestCase(unittest.TestCase):
    ...
    def test_random_tensor_dense(self):
        [T,O] = stsrs.init_rand(order=3,s=10,R=5,sp_frac=1.)
        A = []
        for i in range(T.ndim):
            A.append(ctf.random.random((T.shape[i],5)))

        res = CP_ALS(A,T,O,num_iter=30,sp_res=0,csv_writer=None,Regu=1e-6)
        self.assertTrue(res<=0.5)

    def test_random_tensor_sparse(self):
        [T,O] = stsrs.init_rand(order=3,s=10,R=5,sp_frac=0.99)
        A = []
        for i in range(T.ndim):
            A.append(ctf.random.random((T.shape[i],5)))
