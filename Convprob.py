            ...
            for j in range(num_init):
                ...
                if tenpy.is_master_proc():
                    ...
                    if csv_writer is not None:
                        if method != 'NLS':
                            csv_writer.writerow(
                                [R, k, j, i, t_all, res, converged])
                        else:
                            csv_writer.writerow(
                                [R, k, j, total_iters, t_all, res, converged])
...
if __name__ == "__main__":
    ...
    # Set up CSV logging
    csv_path = join(results_dir,
                    'Convprob' + arg_defs.get_prob_file_prefix(args) + '.csv')
    ...
    if tenpy.is_master_proc():
        ...
        if is_new_log:
            csv_writer.writerow([
                'R', 'problem', 'initialization', 'iterations', 'time',
                'residual', 'converged'
            ])

    convprob(tenpy, tensor, s, f_R, l_R, num_iter, num_gen, csv_writer,
             num_init, args.method, num, Regu, cg_tol, conv_tol)
