...
def norm_cols(A):
    norms = ctf.sum(A**2,axis=0)**.5
    return [A / norms, norms]

def solve_sys(G, RHS):
    t0 = time.time()
    [U,S,VT] = ctf.svd(G)
    #S += (ctf.sum(S**.5)/10.)*ctf.ones(S.shape)
    S = 1./S
    X = ctf.dot(RHS, U)
...
def get_residual_sp(O,T,A,B,C):
    ...
    nrm3 = ctf.vecnorm(T - ctf.TTTP(O,[A*z,B,C]))**2
...
def dt_ALS_step(T,A,B,C,z):
    TC = ctf.einsum("ijk,ka->ija",T,C*z)
    RHS = ctf.einsum("ija,ja->ia",TC,B)
    A = solve_sys(compute_lin_sys(B*z,C), RHS)
    RHS = ctf.einsum("ija,ia->ja",TC,A)
    B = solve_sys(compute_lin_sys(A*z,C), RHS)
    RHS = ctf.einsum("ijk,ia,ja->ka",T,A,B*z)
    C = solve_sys(compute_lin_sys(A*z,B), RHS)
...
def build_leaves(T,A,B,C,z):
    TC = ctf.einsum("ijk,ka->ija",T,C*z)
    RHS_A = ctf.einsum("ija,ja->ia",TC,B)
    RHS_B = ctf.einsum("ija,ia->ja",TC,A)
    RHS_C = ctf.einsum("ijk,ia,ja->ka",T,A,B*z)
...
def lowr_msdt_step(T,A,B,C,z,RHS_A,RHS_B,RHS_C,r,ul="update_leaves"):
    G = compute_lin_sys(B,C)
    ERHS_A = RHS_A - ctf.dot(A*z, G)
    ...
    return [A,B,C,z,RHS_A,RHS_B,RHS_C]
...
def test_rand_naive(s,R,num_iter,sp_frac,sp_res):
    ...
    for i in range(num_iter):
        if sp_res:
            res = get_residual_sp(O,T,A*z,B,C)
        else:
            res = get_residual(T,A*z,B,C)
        if ctf.comm().rank() == 0:
            print("Residual is", res)
        t0 = time.time()
        [A,B,C,z] = dt_ALS_step(T,A,B,C,z)
        t1 = time.time()
        if ctf.comm().rank() == 0:
...
def test_rand_lowr(s,R,r,num_iter,num_lowr_init_iter,sp_frac,sp_ul=False,sp_res=False):
    ...
    #z = zA*zB*zC
    z = ctf.ones(R)
    [RHS_A,RHS_B,RHS_C] = build_leaves(T,A,B,C,z)
    time_lowr -= time.time()
    for i in range(num_iter-num_lowr_init_iter):
        ...
        t0 = time.time()
        if sp_ul:
            [A,B,C,z,RHS_A,RHS_B,RHS_C] = lowr_msdt_step(T,A,B,C,z,RHS_A,RHS_B,RHS_C,r,"update_leaves_sp")
        else:
            [A,B,C,z,RHS_A,RHS_B,RHS_C] = lowr_msdt_step(T,A,B,C,z,RHS_A,RHS_B,RHS_C,r)
        t1 = time.time()
        if ctf.comm().rank() == 0:
